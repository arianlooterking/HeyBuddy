...
- `HotkeyManager(AppSettings)` exposes `ActionInvoked(ShortcutAction, HotkeyGesture)` and `PointerClicked(System.Drawing.Point)`. Call `Start()` on the UI thread with a running message pump and dispose when reconfiguring shortcuts or exiting. Events are posted to the captured synchronization context. Press/release and 400 ms double-taps are distinct. Each action accepts a distinct bare keyboard key or a modifier combination. A matched bare key is consumed globally while HeyBuddy runs. Plain Escape and Tab retain Settings cancellation/navigation, modifier-only and non-keyboard pseudo-keys are refused, and injected input does not trigger global shortcuts or walkthrough progression.
