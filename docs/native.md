...
The current release is 0.2.3. Its four global actions accept either one chosen keyboard button or a key combination, with left/right Shift, Ctrl, and Alt available separately. Voice shortcuts support a quick tap to start and a second tap to finish, or hold and release. Valid shortcut changes persist immediately, and the Agent shortcut restores and focuses the Agent-mode composer. The app picker and exact-name launch/activation paths are included; the most recent installed computer-action evidence remains the 0.2.0 checks that brought existing Calculator and Telegram windows forward and typed an approved marker into a named Notepad document. The broader cross-application input matrix remains incomplete.
