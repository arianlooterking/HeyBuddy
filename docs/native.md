...
The current release is 0.3.0. Its four global actions accept either one chosen keyboard button or a key combination, with left/right Shift, Ctrl, and Alt available separately. Voice shortcuts support a quick tap to start and a second tap to finish, or hold and release. Talk remembers the previously focused application and can attach a transient private capture plus an accessibility map to that request. Valid shortcut changes persist immediately, and the Agent shortcut restores and focuses the Agent-mode composer. The app picker and exact-name launch/activation paths are included; the broader cross-application input matrix remains incomplete.
...
- `WindowsDesktopTools.ObserveWindow(handle, capture, cancellationToken)` aligns up to 120 named, enabled, non-password accessibility controls with the same normalized coordinates as the transient screenshot. The current window/snapshot/element IDs remain short-lived and retain the executor's identity checks.
- `HotkeyManager(AppSettings)` exposes `ActionInvoked(ShortcutAction, HotkeyGesture)` and `PointerClicked(System.Drawing.Point)`. Call `Start()` on the UI thread with a running message pump and dispose when reconfiguring shortcuts or exiting. Events are posted to the captured synchronization context. Press/release and 400 ms double-taps are distinct hook events; the shell maps a quick voice tap to latched listening and a 500 ms hold to release-to-finish. Each action accepts a distinct ordinary key, a specific left/right Shift, Ctrl or Alt button, or a standard modifier-plus-key combination. A matched button is consumed globally while HeyBuddy runs. Plain Escape and Tab retain Settings cancellation/navigation, standalone Windows keys and non-keyboard pseudo-keys are refused, and injected input does not trigger global shortcuts or walkthrough progression.
...
Every input requires the target to be foreground immediately before the operation. Clicks require a UIA clickable point and confirm that the actual window beneath that point is the expected window. Buttons use UIA InvokePattern when available; toggles and selectable items use their matching patterns with state verification. A separate click-through ring shows the intended point without granting input capability. Controls without those patterns use verified foreground input and restore the user's pointer when it remained at the action point. Text entry first uses an editable UIA ValuePattern and compares the resulting value. The refinement adds visible Unicode input for an explicitly editable TextPattern Edit/Document control, with exact element focus, held-modifier checks, and unchanged foreground/cancellation checks between batches of at most 32 UTF-16 code units. This fallback leaves the clipboard untouched and verifies the resulting text. Scroll requires a ScrollPattern; unsupported controls receive an error. Keyboard input is limited to navigation/editing keys and selected simple chords.
...
The separate `--render` check passes without manipulating desktop focus. It verifies actual annotation rasterization at 1200×800, preservation of a negative monitor origin, the stroke's expected pixel coordinates, preservation of unmarked source pixels, click-target geometry, model-coordinate alignment to accessible controls, and an accessibility-backed location fallback. `sketch-composition.png` was visually inspected; `sketch-result.json` records the assertions.
...
For 0.2.1, the Settings fixture passes 54/54 checks without installing a global hook or sending physical input. It round-trips bare `A`, `D7`, `Space`, `Enter`, `Home`, `PageDown`, `Left`, and `OemPlus` through the real recorder and parser, while checking release handling, duplicates, modifier aliases, sided modifiers, mouse/synthetic pseudo-keys, reserved Windows combinations, and the rule that a repeat cannot become a shortcut after its initial key-down did not match. This verifies the configuration and hook-state path; it is not a live physical global-key or hold/double-tap claim.

For 0.2.2, the same fixture passes 82/82 checks. It adds exact recording and parsing for Left Shift, Right Shift, Left Ctrl, Right Ctrl, Left Alt, and Right Alt; proves exact-side matching for all six choices; exercises press/release consumption; covers Right Alt on the AltGr Control-plus-Alt sequence; and rejects standalone-modifier/combo overlaps that would start two actions. No physical keyboard input is sent, so live global standalone-modifier behavior remains a separate acceptance check.

For 0.2.3, the Settings fixture passes 94/94 checks. It adds quick-tap latching, second-tap finish, held-release finish, Agent composer preparation, immediate shortcut persistence, and microphone-level companion reactions. The companion fixture passes 16/16 and renders the compact listening state at 96, 144, and 192 DPI. An independent live probe received 99 frames and a recognized outcome from the selected USB microphone; its approximately −42 dB maximum dynamic level confirms that the source is quiet and does not establish broad transcription accuracy.

For 0.3.0, the Settings fixture passes 100/100 checks. It adds Talk-time focused-app context, typed screen-intent capture, screen-coaching settings, and an explicit proof that location guidance cannot authorize a click. The owned-window fixture maps named controls into screenshot coordinates and invokes its exact button through UI Automation while emitting the separate action marker. The render check covers guidance hit targets, coordinate alignment, and fallback location circles. The installed local Qwen model also returned a valid parsed pointer from the sanitized fixture plus accessibility context.
