...
<#
.SYNOPSIS
Validates and installs the repository's HeyBuddy 0.3.0 release while preserving local user data.

.DESCRIPTION
Run from a normal, unelevated PowerShell 7 session after scripts/release.ps1 has produced the
0.3.0 artifacts. The installer is per-user. This helper never changes settings or credentials.
